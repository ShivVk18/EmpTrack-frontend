...
import { sessionTypes } from "@/lib/enumUtils";
...
function LeaveApplicationForm() {
  ...
  const { leavePolicies, loading } = useLeavePolicies(); 
  ...
  const onSubmit = (data) => {
    submitForm({
      data,
      endpoint: editId ? `/leaveApplication/${editId}` : "/leaveApplication/",
...
/>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormInput name="fromDate" type="date" label="From Date" required />
              <FormInput name="toDate" type="date" label="To Date" required />
            </div>
            ...
            {isHalfDay && (
              <FormSelect
                name="session"
                label="Session"
                options={sessionTypes} // e.g. [{ value: "MORNING", label: "Morning" }]
                required
              />
            )}
